...
export function matchesView(item, view, now = new Date()) {
  ...
  if (view==='today') return !item.completed && displayDate(item)===today;
...
export function filterItems(items, filters, now = new Date()) {
  ...
  return items.filter(t => matchesView(t,filters.view,now)
    ...
    const due=t=>t.due_at?new Date(t.due_at).getTime():t.due_date?new Date(t.due_date+'T23:59:59').getTime():Infinity;

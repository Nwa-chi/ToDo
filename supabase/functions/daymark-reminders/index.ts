...
async function rpc(action:string,job?:string,status?:number){
 const {data,error}=await db.rpc('daymark_push_worker',{p_action:action,p_job:job??null,p_status:status??null});
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  stage='claim';const jobs=await rpc('claim');let accepted=0,failed=0;
  ...
  for(let start=0;start<jobs.length;start+=5)await Promise.all(jobs.slice(start,start+5).map(async(job:any)=>{
   ...
   if(allowed(job.subscription?.endpoint))try{
    const result=await webpush.sendNotification(job.subscription,JSON.stringify({title:'Daymark reminder',body:'A plan is due. Open Daymark to view it.',tag:job.plan_id}),{TTL:3600,urgency:'high',timeout:8000});status=result.statusCode;
   }catch(error:any){status=Number(error.statusCode)||503;}
   await rpc('finish',job.id,status);if(status>=200&&status<300)accepted++;else failed++;
  }));
